...
", "Mgroup1 <- subset(dataset, Pclass == 1 & Sex == 'male')", " Mmedian1 <- median(Mgroup1$Age, na.rm = TRUE)","if (dataset$Sex[i] == 'male' && is.na(dataset$Age[i])) {
  ...
  output$milan_mosaicplot <- renderPlot({
    ...
    vcd::mosaic(
      ...
      highlighting_fill = c( "tomato","lightblue"),
      main = paste("Mosaicplot: ", input$demo_radio , " vs Survived"),
      labeling_args = list(
        set_varnames = c(Survived = "Status"),
        set_labels = list(Survived = c( "Not Survived","Survived"))
      ),
  ...
  output$milan_mosaicplot2 <- renderPlot({
    ...
    vcd::mosaic(
      ...
      main = paste("Mosaicplot: ", inp , " vs Embarked"),
      labeling_args = list(
